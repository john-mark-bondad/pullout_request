...
$columns = array(
  ...
  5=>'cancelled_date',  //pending_date, dispending_date, cancelled_date
  6=>'(select * from across_p_user_account as a
  LEFT JOIN across_p_person as b ON a.eid=b.eid
  where  a.accountid=$cancelled_by)',   //pending_by, dispending_by, cancelled_by
  7=>'status', //status
  8=>'',
  9=>'',
  10=>'',
...
$q1=$this->db->query("
select x.pullout_id as ap_pullout_id, pullout_num, x.date_requested, x.saved_by as saved_by, x.cancelled_date as cancelled_date, x.cancelled_by as cancelled_by, x.description, x.status,
 ...
 c.pid as product_pid, sku, product_name, brand, c.bpid as product_bpid,
  ...
  and a.remark='1' and status='3'  $srch
...
$cancelled_date=$r[$x]['cancelled_date']; 
...
$q2=$this->db->query("
  ...
  ");
$r2=$q2->result_array();
$cancelled_by_name=(count($r2)==0)?"N/A":$r2[0]['lname']." ".$r2[0]['ename'].", ".$r2[0]['fname']." ".$r2[0]['mname'];
...
$data[$x] = array(
  ...
  <input type='hidden' id='tbl_requested_by$x' value='$saved_by_name' />
...
<input type='hidden' id='tbl_pending_pullout_qty$x' value='$pullout_qty'/>
  ...
  4=>"$saved_by_name",
  5=>"$cancelled_date",
  6=>"$cancelled_by_name", // 'saved_by' this is pending status
  7=> "$stat",
  8=>"<button class='btn' style='width:100%; background:rgba(0,0,0,0);' onclick='view_cancelled($x)'><span class='glyphicon glyphicon-search'></span></button>",
